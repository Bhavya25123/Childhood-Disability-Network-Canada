    ...
    <PageLayout>
      ...
      <section className="bg-purple-light/30 py-16 px-8">
        <div className="max-w-screen-xl mx-auto text-center">
          ...
          <Button 
            className="bg-white text-purple-900 hover:bg-gray-50 border border-purple/20 font-semibold py-3 px-8 rounded-lg"
            tabIndex={0}
        ...
        <div className="max-w-screen-xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {supportServices.map((service, index) => (
              <div 
                key={index} 
                className="bg-white p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow border border-purple/5"
                tabIndex={0}
                role="article"
                aria-labelledby={`service-title-${index}`}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    // Could add additional action when card is selected via keyboard, if needed
                    e.preventDefault();
                  }
                }}
              >
                <div className="mb-4" aria-hidden="true">{service.icon}</div>
                <h3 id={`service-title-${index}`} className="text-xl font-semibold mb-2 text-purple-900">{service.title}</h3>
